...
Light::Light(const unsigned sqrtN)
{
	this->sqrtN = sqrtN;

	unsigned nFunctions = sqrtN * sqrtN;
...
void Light::directLight(const Sampler &sampler)
{
	...
	for (unsigned i = 0; i < nFunctions; ++i)
	{
		...
		{
			unrotatedCoeffs[0][i] += lightIntensity(samples[j].theta, samples[j].phi) * samples[j].rbfValues[i];
			unrotatedCoeffs[1][i] += lightIntensity(samples[j].theta, samples[j].phi) * samples[j].rbfValues[i];
			unrotatedCoeffs[2][i] += lightIntensity(samples[j].theta, samples[j].phi) * samples[j].rbfValues[i];
		}
...
void Light::lightFromImage(const char* imagePath, Sampler& sampler, int numBands, float lightIntensity)
{
	...
	for (unsigned i = 0; i < sampler.size(); ++i) {
		...
		for (int j = 0; j < numBands*numBands; ++j) {
			...
			double shFunction = sampler.samples[i].rbfValues[j];
...
{
	rotateSHCoefficients(sqrtN, unrotatedCoeffs[0], rotatedCoeffs[0], theta, phi);
	rotateSHCoefficients(sqrtN, unrotatedCoeffs[1], rotatedCoeffs[1], theta, phi);
	rotateSHCoefficients(sqrtN, unrotatedCoeffs[2], rotatedCoeffs[2], theta, phi);

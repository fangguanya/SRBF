...
void Scene::loadAllData(int nBands)
{
	...
	for (int i = 0; i < nMesh; ++i)
	{
		...
		/*sprintf(path, "./%s/%s", "models", names[i].c_str());
...
void Scene::saveAllData(int sqrtN)
{
	...
	for (int i = 0; i < nObj; ++i)
	{
		...
		for (int j = 0; j < nVer; ++j)
		{
			...
			fwrite(vertices[j].unshadowedCoeffs, sizeof(double), nFuncs, out);
			fwrite(vertices[j].shadowedCoeffs, sizeof(double), nFuncs, out);
		}
...
void Scene::printFirst(const int idx, int n)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		const double *tlm = vertices[ii].unshadowedCoeffs;
...
bool Scene::generateCoeffs(Sampler &sampler, int sqrtN)
{
	if (!initCoeffs(sqrtN))return false;

	
	this->generateDirectCoeffs(sampler, sqrtN);

	return true;
}

bool Scene::initCoeffs(int sqrtN)
{
	const int numFunctions = sqrtN * sqrtN;
...
void Scene::generateDirectCoeffs(Sampler &sampler, int sqrtN)
{
	const unsigned nFunctions = sqrtN * sqrtN;
		...
		for (unsigned verIdx = 0; verIdx < nVertices; ++verIdx, ++curNumVertices)
		{
			...
			for (unsigned sampleIdx = 0; sampleIdx < nSamples; ++sampleIdx)
			{
				...
				if (cosine > 0.0f)
				{
					...
					{
						double contribution = cosine * samples[sampleIdx].rbfValues[l];

...
void PRTRenderer::initGLFW()
{
	...
	window = glfwCreateWindow(width, height, "SRBF", nullptr, nullptr);
	...
	if (lightType == LIGHTING_TYPE_SH_UNSHADOWED)
	{
		for (unsigned objIdx = 0; objIdx < nObjects; ++objIdx)
		{
			...
			for (unsigned verIdx = 0; verIdx < nVertices; ++verIdx)
			{
				...
				unsigned nFunctions = light.sqrtN * light.sqrtN;
				for (unsigned l = 0; l < nFunctions; ++l)
				{
					brightness.r += light.unrotatedCoeffs[0][l] * curVertex.unshadowedCoeffs[l];
					brightness.g += light.unrotatedCoeffs[1][l] * curVertex.unshadowedCoeffs[l];
					brightness.b += light.unrotatedCoeffs[2][l] * curVertex.unshadowedCoeffs[l];
				}
	...
	else if (lightType == LIGHTING_TYPE_SH_SHADOWED)
	{
		for (unsigned objIdx = 0; objIdx < nObjects; ++objIdx)
		{
			...
			for (unsigned verIdx = 0; verIdx < nVertices; ++verIdx)
			{
				...
				for (unsigned l = 0; l < nFunctions; ++l)
				{
					brightness.r += light.unrotatedCoeffs[0][l] * curVertex.shadowedCoeffs[l];
					brightness.g += light.unrotatedCoeffs[1][l] * curVertex.shadowedCoeffs[l];
					brightness.b += light.unrotatedCoeffs[2][l] * curVertex.shadowedCoeffs[l];
				}
